...
# =============================================================================
# VERSION 2 (backup) — active code before image fixes, with invalid-link logic
# =============================================================================
# # curryss.py
# # Python 3.9+
...
#         "#js-product-detail, [data-component='product-detail'], "
#         "[data-test='product-detail-page'], .pdp-container"
#     )
...
#         "[data-component='product-card'], [data-test='product-card'], "
#         ".product-card, .productCard, [class*='ProductCard'], .product-list-item"
#     )
#     has_multiple_products = len(product_cards) >= 3
#     pagination = soup.select_one(
#         "[data-component='pagination'], .pagination, "
#         "[aria-label='Pagination'], nav[aria-label*='page']"
#     )
...
#     not_found_phrases = [
#         "product not found", "page not found", "item no longer available",
#         "this product is no longer available", "sorry, we couldn't find",
#         "this item has been discontinued", "no longer stocked",
#     ]
#     has_not_found_message = any(phrase in page_text for phrase in not_found_phrases)
#     has_filters = soup.select_one(
#         "[data-component='filter'], [data-test='filter'], .filter-panel, "
#         ".facet-navigation, [aria-label*='Filter']"
#     ) is not None
...
#             ".productDescription", ".pdp-description",
#         ):
...
#     TEST_URL = "https://www.currys.co.uk/products/laura-ashley-vqsbt583bsuk-4slice-toaster-elveden-navy-10237286.html"
...
# =============================================================================
# VERSION 3 (ACTIVE) — image fixes applied
# FIXES:
#   - _upgrade_currys_image: ensure absolute before regex, append preset if missing
#   - _is_pdp_image: removed range(0,7) suffix cap (Currys uses _007.._022+)
#   - _collect_images: selector .carouselindicators → .carouselitem (actual HTML class)
#                      dedupe by base URL instead of full URL
#                      added raw HTML regex fallback
# =============================================================================
...
def _upgrade_currys_image(u: str) -> str:
    # FIX: ensure absolute first, then swap thumbnail preset, then append large preset if missing
    u = _absolute(u, "https://www.currys.co.uk")
    u = re.sub(r"\$t-[^$]*\$", "$l-large$", u)
    if "media.currys.biz" in u and "$l-large$" not in u and "?" not in u:
        u = u + "?$l-large$&fmt=auto"
    return u
...
def _is_pdp_image(url: str, pid: str) -> bool:
    ...
    Accept only:
      ...
      https://media.currys.biz/i/currysprod/<PID>_<NNN>  (any 3-digit suffix)
    FIX: removed range(0,7) restriction — Currys pages have images like _007, _017.._022
    """
    url = _absolute(url)
    if not url.startswith("https://media.currys.biz/i/currysprod/"):
        return False
...
def _collect_images(soup: BeautifulSoup, pid: str) -> List[str]:
    """
    FIX: selector changed from .carouselindicators (wrong) to .carouselitem (actual class).
    FIX: dedupe by base URL (before ?) so slick-cloned slides don't produce duplicates.
    ADDED: raw HTML regex fallback for resilience.
    Returns images in carousel order.
    """
    cand: List[str] = []

    # Primary: carousel slide items (actual class used by Currys)
    for el in soup.select(".carouselitem img[src], .carouselitem source[srcset]"):
        u = el.get("src") or el.get("srcset") or ""
        if not u:
            continue
    ...
    # Fallback: product gallery data-component
    if not cand:
        for el in soup.select("[data-component='product-gallery'] img[src], [data-component='product-gallery'] source[srcset]"):
            u = el.get("src") or el.get("srcset") or ""
            if not u:
    ...
    # Raw HTML regex fallback (most resilient — searches the raw HTML string)
    if not cand:
        raw_urls = re.findall(
            rf"https://media\.currys\.biz/i/currysprod/{pid}(?:_\d{{3}})?[?\"'&][^\"'<>\s]*",
            str(soup)
        )
        for u in raw_urls:
            u = u.rstrip("\"'&")
    ...
    # Whitelist & de-duplicate by canonical base URL (preserves first occurrence order)
    seen_core, whitelisted = set(), []
    for u in cand:
        if _is_pdp_image(u, pid):
            core = u.split("?")[0]
